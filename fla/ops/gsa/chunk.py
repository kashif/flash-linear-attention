...
def chunk_gsa_bwd_k_kernel_dA(
    ...
    desc_dA = make_tensor_descriptor(dA+((i_v*all+bos)*HQ+i_hq)*BT, [T, BT], [HQ*BT, 1], [BC, BC])
    ...
    if i_i > i_j:
        ...
        desc_g = make_tensor_descriptor(g + (bos*H+i_h) * V, [T, V], [H*V, 1], [BC, BV])
        desc_do = make_tensor_descriptor(do + (bos*HQ+i_hq) * V, [T, V], [HQ*V, 1], [BC, BV])
        # [BV,]
        b_gn = tl.load(p_gn, mask=m_v, other=0.)
        # [BC, BV]
        b_g = desc_g.load([i_t*BT + i_i*BC, i_v*BV])
...
def chunk_gsa_bwd_k_kernel_intra_dvg(
    ...
    for i_j in range(i_i + 1, NC):
        ...
        b_do = desc_do.load([i_t*BT + i_j*BC, i_v*BV]) * tl.where(m_j[:, None], exp2(b_g - b_gn[None, :]), 0)
        # [BC, BC]
    ...
    desc_v = make_tensor_descriptor(v + (bos*H+i_h)*V, [T, V], [H*V, 1], [BC, BV])
    desc_do = make_tensor_descriptor(do + (bos*HQ+i_hq)*V, [T, V], [HQ*V, 1], [BC, BV])
    desc_dv = make_tensor_descriptor(dv + (bos*HQ+i_hq)*V, [T, V], [HQ*V, 1], [BC, BV])
    desc_dg = make_tensor_descriptor(dg + (bos*HQ+i_hq)*V, [T, V], [HQ*V, 1], [BC, BV])
