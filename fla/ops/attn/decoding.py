...
def naive_attn_decoding_kernel(
    ...
    desc_q = make_tensor_descriptor(q + i_bh * K, [K], [1], [BK])
    desc_o = make_tensor_descriptor(o + i_bh * V, [V], [1], [BV])
    ...
    b_o = b_o / b_acc
    desc_o.store([i_v * BV], b_o.to(desc_o.dtype))
